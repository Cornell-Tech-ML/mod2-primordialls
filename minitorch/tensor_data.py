...
def to_index(ordinal: int, shape: Shape, out_index: OutIndex) -> None:
    ...
    """
    cont = strides_from_shape(list(shape))
    for i in range(len(shape)):
        out_index[i] = ordinal // cont[i]
        ordinal = ordinal % cont[i]
...
def shape_broadcast(shape1: UserShape, shape2: UserShape) -> UserShape:
    ...
    for i in range(len1):
        if sh1[i] == sh2[i]:
            out_shape.append(sh1[i])
        elif sh1[i] == 1:
            out_shape.append(sh2[i])
        elif sh2[i] == 1:
            out_shape.append(sh1[i])
        else:
...
class TensorData:
    ...
    def permute(self, *order: int) -> TensorData:
        ...
        return TensorData(
            own[0].copy(),
            tuple([own[1][i] for i in order]),
            tuple([own[2][i] for i in order]),
        )
